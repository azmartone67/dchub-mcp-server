...
//
// DISTANCES. A distance between a facility and a point the caller chose says
// where the facility is, so inside a record being coarsened every distance is
// coarsened with it: served in whole km (never below 1), whole miles (never
// below 1) or thousands of metres (never below 1000), and the record carries
// distance_status "approximate_1km". When the backend also sent the figure
// measured from the facility's 2dp point — a `<key>_approx` twin beside the
// key, e.g. km + km_approx on new_facilities_nearby — the twin is what is
// served: it agrees with the 2dp coordinates this caller gets for the same
// facility. The twin never reaches a coarsened caller as a second field.
// distance_status is stamped on every record whose distances were served
// coarsened, whether or not the rounding moved the value — its absence must
// not say that the exact figure was already a whole number.
//
...
export const DISTANCE_APPROX_STATUS = 'approximate_1km';
const APPROX_TWIN_SUFFIX = '_approx';
...
// Distance keys by unit (after normKey, with any `_approx` suffix removed).
// `km` alone is the backend's new_facilities_nearby spelling.
const DIST_KM_KEY_RE = /^(?:kms?|(?:.*_)?dist(?:ance)?_kms?)$/;
const DIST_MI_KEY_RE = /^(?:mi|miles|(?:.*_)?dist(?:ance)?_(?:mi|miles))$/;
const DIST_M_KEY_RE = /^(?:meters|metres|(?:.*_)?dist(?:ance)?_(?:m|meters|metres))$/;
/** The serving step of a distance key (1 km, 1 mi, 1000 m), or 0 if it is not one. */
function distanceStep(k) {
  let nk = normKey(k);
  if (nk.endsWith(APPROX_TWIN_SUFFIX)) nk = nk.slice(0, -APPROX_TWIN_SUFFIX.length);
  if (DIST_KM_KEY_RE.test(nk) || DIST_MI_KEY_RE.test(nk)) return 1;
  if (DIST_M_KEY_RE.test(nk)) return 1000;
  return 0;
}
...
// A distance as served to a coarsened caller: a whole number of `step` units,
// never below one step. Anything that is not a non-negative number is withheld.
function coarsenDistanceValue(v, step) {
  let n = NaN;
  if (typeof v === 'number') n = v;
  else if (typeof v === 'string' && /^\s*\d+(?:\.\d+)?\s*$/.test(v)) n = Number(v);
  if (!Number.isFinite(n) || n < 0) return { drop: true };
  const q = Math.max(step, Math.round(n / step) * step);
  return { value: typeof v === 'string' ? String(q) : q };
}

export function isCoordinateKey(k) { return isCoordKey(k); }
export function isDistanceKey(k) { return distanceStep(k) > 0; }
  ...
  const visit = (node, depth, ctx) => {
    ...
    for (const k of Object.keys(node)) {
      ...
      if (inFacility && !exact) {
        ...
        const step = v !== null && typeof v !== 'object' ? distanceStep(k) : 0;
        if (step) {
          // A twin is consumed by its key: served in its place, never beside it.
          if (k.endsWith(APPROX_TWIN_SUFFIX)
              && Object.prototype.hasOwnProperty.call(node, k.slice(0, -APPROX_TWIN_SUFFIX.length))) {
            stats.removed += 1; distServed = true;
            continue;
          }
          const twin = node[k + APPROX_TWIN_SUFFIX];
          const r = coarsenDistanceValue(twin !== undefined && twin !== null ? twin : v, step);
          stats.rounded += 1; distServed = true;
          if (r.drop) { stats.removed += 1; continue; }
          out[k] = r.value;
          continue;
        }
    ...
    if (distServed) out.distance_status = DISTANCE_APPROX_STATUS;
    seen.delete(node);
    return out;
  };
