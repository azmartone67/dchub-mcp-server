...
export const DIRECTORY_TOOLS = Object.freeze({
  ...
  subscribe_digest: 'Subscribe an email address to DC Hub\'s weekly digest of market changes. DC Hub emails a confirmation link first; nothing is sent until it is confirmed, and every digest has an unsubscribe link.',
  ...
  analyze_site: 'Analyze one lat/lon for data-center suitability: power, gas, fiber, market and risk factors.',
  get_composite_site_score: 'Score a lat/lon 0-100 for data-center suitability over measured factors only (power, fiber, hazards, water), with a coverage map of what was and was not measured.',
  ...
  generate_site_analysis: 'Generate a multi-page Site Analysis PDF for one lat/lon and capacity.',
  compare_sites: 'Compare 2-4 candidate locations side by side across power, gas, fiber, market and risk, and rank them.',
  ...
  source_capacity: 'Search DC Hub Capacity Source listings of data-center capacity to buy or lease, by size (kW or MW) and location.',
});

export const DIRECTORY_INSTRUCTIONS =
  'DC Hub provides data on data-center facilities, markets, power grids, gas, fiber, '
  + 'site risk and deals. Tools are lookups, except subscribe_digest, which emails a '
  + 'confirmation link to an address the user provides. Results without a DC Hub plan '
...
const DROP_KEY = new RegExp('^(' + [
  ...
  'credits?(_.*)?', 'pro_.*', 'developer_(url|usd.*|hint)', 'enterprise_(url|usd.*)', 'pack_.*', 'buy.*',
...
export function scrubToolResult(result) {
  if (!result || typeof result !== 'object') return result;
  const gated = isGated(result);
  const out = {};
  const content = [];
...
export function transformDirectoryMessage(msg, method) {
...
export function transformDirectoryBody(text, method) {
  if (typeof text !== 'string' || !text) return text;
  const trimmed = text.trimStart();
  if (trimmed[0] === '{' || trimmed[0] === '[') {
    try {
      const j = JSON.parse(trimmed);
      const out = Array.isArray(j) ? j.map((m) => transformDirectoryMessage(m, method)) : transformDirectoryMessage(j, method);
  ...
  if (/(^|\n)data:/.test(text)) {
    return text.split(/\n\n/).map((ev) => {
      ...
      if (!data) return ev;
      let out;
      try { out = JSON.stringify(transformDirectoryMessage(JSON.parse(data), method)); } catch (_) { out = scrubText(data); }
...
export function installDirectoryResponseFilter(req, res) {
  ...
  res.end = (chunk, enc, cb) => {
    ...
    let body;
    try { body = transformDirectoryBody(raw, method); } catch (e) {
