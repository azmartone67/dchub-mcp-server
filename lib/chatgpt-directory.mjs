...
//   outputSchemaEnv  string                   — env flag; '1' serves outputSchema
...
export function createDirectoryProfile(cfg) {
  ...
  // Tools held to OpenAI's connector contract: exactly one text item, never
  // the plans line. Default = search + fetch (the ChatGPT profile). A profile
  // with no such contract passes [] so a partial search/fetch closes with the
  // same neutral line as every other gated tool (Grok audit of /mcp/claude,
  // 2026-09-26: keyless search returned a 3-of-5 partial_preview with none).
  const CONNECTOR = cfg.connectorTools ? new Set(cfg.connectorTools) : CONNECTOR_TOOLS;
  ...
  function scrubToolResult(result, tool) {
    ...
    const connector = CONNECTOR.has(tool);
  ...
  // r-directory-output-schema (2026-09-26): the canonical outputSchema projected
  // through the same transforms scrubValue applies to results, so the scrubbed
  // structuredContent validates against what the profile serves
  // (test/directory-output-schema-contract.test.mjs):
  //   - keys scrubValue drops are dropped (DROP_KEY, removed-tool keys, dotted
  //     paths with a dropped segment)
  //   - keys scrubValue renames are renamed (_in_pro → _withheld, …)
  //   - descriptions go through scrubText, then SCHEMA_STEER: schema text says
  //     what a field is, never what to do with it (the directory fair-play rule)
  //   - a gated result gains `notice` (a string) at the top level
  // Canonical schemas are loose objects with no `required` (measured 2026-09-26:
  // additionalProperties {} on all 92), which is what lets a trimmed preview
  // validate; any `required` that survives is filtered through the same drops.
  const SCHEMA_STEER = /\b(when citing|how to attribute|cite (dc hub|this)|quote the|call [a-z_]+\s*\(|call this|prefer)\b/i;
  function schemaDescription(d) {
    const s = scrubText(d);
    if (!s) return '';
    return s.split(/(?<=[.!?;])\s+/).filter((p) => !SCHEMA_STEER.test(p)).join(' ').trim();
  }
  function scrubOutputSchema(schema) {
    if (!schema || typeof schema !== 'object') return schema;
    if (Array.isArray(schema)) return schema.map(scrubOutputSchema);
    const out = {};
    for (const [k, v] of Object.entries(schema)) {
      if (k === '$schema' || k === '_meta') continue;
      if (k === 'properties' && v && typeof v === 'object') {
        const props = {};
        for (const [pk, pv] of Object.entries(v)) {
          if (DROP_KEY.test(pk) || REMOVED_SET.has(pk)) continue;
          if (pk.includes('.') && pk.split('.').some((seg) => DROP_KEY.test(seg.replace(/\[\d+\]$/, '')))) continue;
          props[renameKey(pk)] = scrubOutputSchema(pv);
        }
        out.properties = props;
      } else if (k === 'required' && Array.isArray(v)) {
        const req = v.filter((r) => !DROP_KEY.test(r) && !REMOVED_SET.has(r)).map(renameKey);
        if (req.length) out.required = req;
      } else if (k === 'description' && typeof v === 'string') {
        const d = schemaDescription(v);
        if (d) out.description = d;
      } else {
        out[k] = scrubOutputSchema(v);
      }
    }
    return out;
  }
  function directoryOutputSchema(schema) {
    const base = scrubOutputSchema(schema && typeof schema === 'object' ? schema
      : { type: 'object', properties: {}, additionalProperties: {} });
    const props = { ...(base.properties || {}) };
    const noticeSchema = { type: 'string', description: 'Present when part of the result is withheld: one line naming where the full result is available.' };
    props.notice = props.notice ? { anyOf: [props.notice, noticeSchema] } : noticeSchema;
    return { ...base, type: 'object', properties: props };
  }
  // Off by default: the served tool definitions are what a directory reviews,
  // so the schemas ship dark and are switched on with a new submission.
  const outputSchemaOn = () => !!(cfg.outputSchemaEnv && process.env[cfg.outputSchemaEnv] === '1');
  ...
  // served only with the profile's flag on (see directoryOutputSchema above);
  // with it off the listing is the one the directory reviewed.
  function directoryToolsList(result) {
    const withOutput = outputSchemaOn();
    ...
    for (const [name, description] of Object.entries(TOOLS)) {
      ...
      if (withOutput) entry.outputSchema = directoryOutputSchema(t.outputSchema);
  ...
  return Object.freeze({
    ...
    scrubText, scrubStructured, scrubToolResult, scrubSchema, directoryToolsList, directoryOutputSchema,
...
const _CHATGPT = createDirectoryProfile({
  ...
  // OFF until the next OpenAI submission: 1.0.0 is in review with no schemas.
  outputSchemaEnv: 'CHATGPT_DIRECTORY_OUTPUT_SCHEMA',
});
