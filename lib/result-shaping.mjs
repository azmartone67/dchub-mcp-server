...
// ── scForStamp: the ONE safe way for a decorator to get an sc to write on ────
//
// THE BUG THIS EXISTS TO END (measured live 2026-09-02 on search_facilities).
// A decorator that wants to add ONE metadata key wrote:
//
//     const sc = (result.structuredContent && typeof result.structuredContent === 'object')
//       ? { ...result.structuredContent } : {};        // ← fabricates a data-less sc
//     sc._bind = { … };
//     return { ...result, structuredContent: sc };
//
// For the ~40 tools whose handler returns data ONLY in content[0].text, that
// `: {}` MINTS a structuredContent that never had the payload. Every later
// decorator then adds its own key to that same empty object, and the response
// ships with two halves that disagree:
//
//     content[0].text     → {"data":[…10 rows…],"success":true,"pagination":{…}}
//     structuredContent   → {_bind,_entity,citation,identity,provenance,quota}
//
// Captured 7/7 times in one run. There is no `error`, no `isError`, no
// `success:false` — a client that prefers structuredContent (Claude Desktop /
// Claude.ai, and any SDK client, because it is the well-formed machine surface)
// reads ZERO rows and cannot tell that from "no facilities matched".
//
// This is the SAME defect class as the 2026-06-21 next_session fix at the top of
// this file, reintroduced through a different decorator — which is the argument
// for a shared helper rather than a third one-off repair. withNextSession's
// mirror does not rescue it: that mirror only runs as a SIDE EFFECT of stamping
// the nudge, and it `return result`s (discarding the merge) whenever the nudge
// is idempotent-skipped or its factory yields nothing.
//
// The contract: return an object safe to stamp, or NULL meaning "there is
// nothing here to attach metadata to without hiding the payload — skip your
// stamp and leave structuredContent absent, so the client falls back to
// content[] and the DATA still shows." Losing a nudge is a nuisance; losing the
// payload is the product.
export function scForStamp(result) {
  try {
    const existing = result && result.structuredContent;
    if (existing && typeof existing === 'object' && !Array.isArray(existing)) {
      // The handler set one — it is authoritative, copy it as-is.
      return { ...existing };
    }
    // No sc: mirror the real payload out of content[0] instead of inventing one.
    const payload = payloadObjFromContent(result && result.content);
    return payload ? { ...payload } : null;   // null → stay content-only
  } catch (_) {
    return null;
  }
}
