...
| 🔒 PRO | Returns 402 `upgrade_required` — requires PRO tier ($99/mo) |
...
| 15 | `save_site` | 🔒 PRO | `{lat:39.04, lon:-77.48}` → 402 `upgrade_required` (PRO $99/mo) |
| 16 | `list_saved_sites` | 🔒 PRO | `{}` → 402 `upgrade_required` (PRO $99/mo) |
| 17 | `set_market_alert` | ✅ WORKS | `{market:"northern-virginia", channel:"webhook", destination:"https://…"}` → `{ok:true, subscription_id}` |
| 18 | `export_dataset` | 🔒 PRO | `{format:"csv"}` → 402 `upgrade_required` (PRO $99/mo) |
...
| 🔒 PRO ($99/mo) | 3 | save_site, list_saved_sites, export_dataset |
