...
**DC Hub** is live data-center, power-grid & gas intelligence for AI agents — **91 MCP tools**, 21,800+ facilities, DCPI market scores, grid telemetry across 10 ISOs, interconnection queues, fiber, and M&A. **Free tier, no signup.** Every response cites `dchub.cloud`.
...
| Method | Best for | How |
|---|---|---|
| **1. MCP server** (recommended — all 91 tools) | Claude Desktop, Cursor, Cline, Continue, Windsurf, Zed, any MCP client | Point your client at **`https://dchub.cloud/mcp`** (Streamable HTTP). See [`mcp-clients/`](./mcp-clients/). |
